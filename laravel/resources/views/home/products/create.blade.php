                    ...
                    <div class="sngl-top">
                        ...
                        <div class="col-md-7 single-top-right">
                            <div class="single-para simpleCart_shelfItem">
                                <h2>{{Form::text('name', null, array('value' => old('name'), 'class' => 'form-control', 'aria-describedby' => 'name-category', 'placeholder' => 'Enter product name', 'required'))}}</h2>
                                @if ($errors->has('name'))
                                    <span class="help-block has-error">
                                ...
                                <div class="border-top-buttom">
                                    ...
                                    <h5>
                                        <div class="form-group{{ $errors->has('price') ? ' has-error' : '' }}" style="margin-bottom: 0px;">
                                                ...
                                                <div class="col-md-4">
                                                    {{Form::text('price', null, array('value' => old('price'), 'class' => 'form-control col-md-4', 'aria-describedby' => 'price', 'placeholder' => 'Enter price', 'required'))}}
                                                </div>
                                        </div>
                                    </h5>
                                ...
                                <p>{{Form::textarea('description', null, array('value' => old('description'), 'style' => 'margin-bottom: 10px', 'class' => 'form-control col-md-4', 'aria-describedby' => 'description', 'placeholder' => 'Enter description', 'required'))}}</p>
                                ...
                                <div class="row form-group{{ $errors->has('category') ? ' has-error' : '' }}" style="margin-bottom: 0px; margin-top: 10px;">
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        @endphp
                                        {{Form::select('category', $assoc, old('category'), array('placeholder' => 'Please select a category','onChange' => 'update_categoty()','class' => 'form-control ','aria-describedby' => 'category', 'required'))}}
                                ...
                                <div id="characteristics">
                                    @if(old('category') != null)
                                        @php
                                            $category = Category::where('name', old('category'))->first();
                                        @endphp
                                        @if($category != null)
                                            @foreach(json_decode($category->json_characteristics) as $key => $char)
                                                <div class="row form-group{{ $errors->has('char'.$key) ? ' has-error' : '' }}" style="margin-bottom: 0px; margin-top: 10px;">
                                                    <label for="category" class="col-md-2 control-label">@if(!is_array($char)) {{$char}} @else {{$char[0]}} @endif</label>
                                                    <div class="col-md-8">
                                                        @if(!is_array($char))
                                                            <input value = "{{old('char'.$key)}}" class="form-control" aria-describedby="sizing-addon2" placeholder="Please enter a {{$char}}" name=char"{{$key}}" type="text">
                                                        @else
                                                            {{Form::select('char'.$key, array_combine($char[1], $char[1]), old('char'.$key), array('placeholder' => 'Please select a '.$char[0],'class' => 'form-control ','aria-describedby' => 'category', 'required'))}}
                                                        @endif
                                                    </div>
                                                </div>
                                            @endforeach
